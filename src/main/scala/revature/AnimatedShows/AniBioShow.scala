...
object AniBioShow {
  ...
  def addingAniBioShow(yourLAShowSelection: Int, a: Int): Unit = {
  ...
  def AniBioShows(ahhh: Int): Unit = {
    ...
    moreOfAAniBioShow(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def AniBioShowsButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
