...
object AniMusicalShow {
  ...
  def addingAniMusicalShow(yourLAShowSelection: Int, a: Int): Unit = {
    ...
    while (resultAdd.next) {
      ...
      if (decide == 1) {
        ...
        addingAniMusicalShow(yourLAShowSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfAAniMusicalShow(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def AniMusicalShows(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfAAniMusicalShow(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def AniMusicalShowsButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    println("ANIMATED MUSCIAL SHOWS:")
    query = "SELECT * FROM series WHERE sLaStatus = 0 and sGenre = 'Muscial'"
    val resultSet = s.executeQuery(query)
    while (resultSet.next) {
      val series_id = resultSet.getString("series_id")
      val sTitle = resultSet.getString("sTitle")
    ...
    if (back == 1) {
      ...
      AniMusicalShowsButNotStuck(huh)
    }
  }
