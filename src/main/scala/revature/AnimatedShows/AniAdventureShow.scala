...
object AniAdventureShow {
  ...
  def addingAniAdventureShow(yourLAShowSelection: Int, a: Int): Unit = {
    ...
    while (resultAdd.next) {
      ...
      if (decide == 1) {
        ...
        val back = readInt()
        if (back == 1) {
          AniAdventureShowsButNotStuck(a)
        }
        else {
          System.exit(0)
        }
        ...
        addingAniAdventureShow(yourLAShowSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfAAniAdventureShow(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def AniAdventureShows(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfAAniAdventureShow(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def AniAdventureShowsButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    if (back == 1) {
      ...
      AniAdventureShows(huh)
    }
    else {
      println("Please enter a valid number (1-3)." + "\n")
      AniAdventureShowsButNotStuck(huh)
    }
  }
