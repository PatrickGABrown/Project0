...
object AniActionShow {
  ...
  def addingAniActionShow(yourLAShowSelection: Int, a: Int): Unit = {
    ...
    while (resultAdd.next) {
      ...
      if (decide == 1) {
        ...
        addingAniActionShow(yourLAShowSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfAAniActionShow(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def AniActionShows(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfAAniActionShow(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def AniActionShowsButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
