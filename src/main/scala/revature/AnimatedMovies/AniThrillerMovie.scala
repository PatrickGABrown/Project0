...
object AniThrillerMovie {
      ...
      if (decide == 1) {
        try{
          ...
          case e: Exception => //e.printStackTrace()
            ...
            val back = readInt()
            if (back == 1) {
              AniThrillerMoviesButNotStuck(a)
            }
            else {
              System.exit(0)
            }
        ...
        addingAniThriller(yourLAMovieSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfAAniThrillerMovie(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def AniThrillerMovies(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfAAniThrillerMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def AniThrillerMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    if (back == 1) {
      ...
      AniThrillerMovies(huh)
    }
    else {
      println("Please enter a valid number (1-3)." + "\n")
      AniThrillerMoviesButNotStuck(huh)
    }
  }
