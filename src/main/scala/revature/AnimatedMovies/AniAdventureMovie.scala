...
object AniAdventureMovie {
      ...
      if (decide == 1) {
        try{
          ...
          case e: Exception => //e.printStackTrace()
            ...
            val back = readInt()
            if (back == 1) {
              AniAdventureMoviesButNotStuck(a)
            }
            else {
              System.exit(0)
            }
        ...
        addingAniAdventure(yourLAMovieSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfAAniAdventureMovie(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def AniAdventureMovies(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfAAniAdventureMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def AniAdventureMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    if (back == 1) {
      ...
      AniAdventureMovies(huh)
    }
    else {
      println("Please enter a valid number (1-3)." + "\n")
      AniAdventureMoviesButNotStuck(huh)
    }
  }
