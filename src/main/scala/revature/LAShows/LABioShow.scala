...
object LABioShow {
  ...
  def addingLABioShow(yourLAShowSelection: Int, a: Int): Unit = {
  ...
  def LABioShows(ahhh: Int): Unit = {
    ...
    moreOfALABioShow(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LABioShowsButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
