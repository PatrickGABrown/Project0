...
object LADramaMovie {
  ...
  def addingLADrama(yourLAMovieSelection: Int, a: Int): Unit = {
  ...
  def LADramaMovies(ahhh: Int): Unit = {
    ...
    moreOfALADramaMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LADramaMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
