...
object LAMusicalMovie {
  ...
  def addingLAMusical(yourLAMovieSelection: Int, a: Int): Unit = {
    ...
    while (resultAdd.next) {
      ...
      if (decide == 1) {
        ...
        addingLAMusical(yourLAMovieSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfALAMusicalMovie(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def LAMusicalMovies(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfALAMusicalMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LAMusicalMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    if (back == 1) {
      ...
      LAMusicalMoviesButNotStuck(huh)
    }
  }
