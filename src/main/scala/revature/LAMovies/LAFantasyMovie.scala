...
object LAFantasyMovie {
  ...
  def addingLAFantasy(yourLAMovieSelection: Int, a: Int): Unit = {
    ...
    while (resultAdd.next) {
      ...
      if (decide == 1) {
        ...
        addingLAFantasy(yourLAMovieSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfALAFantasyMovie(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def LAFantasyMovies(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfALAFantasyMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LAFantasyMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    if (back == 1) {
      ...
      LAFantasyMoviesButNotStuck(huh)
    }
  }
