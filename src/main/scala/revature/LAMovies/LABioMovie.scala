...
object LABioMovie {
  ...
  def addingLABio(yourLAMovieSelection: Int, a: Int): Unit = {
  ...
  def LABioMovies(ahhh: Int): Unit = {
    ...
    moreOfALABioMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LABioMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
