...
object LAAdventureMovie {
      ...
      if (decide == 1) {
        try{
          ...
          case e: Exception => //e.printStackTrace()
            ...
            val back = readInt()
            if (back == 1) {
              LAAdventureMoviesButNotStuck(a)
            }
            else {
              System.exit(0)
            }
        ...
        addingLAAdventure(yourLAMovieSelection, a)
      }
    }
  }

  //Function to add a movie you choose to your watchlist for watching later.
  def moreOfALAAdventureMovie(y: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
  ...
  //LADramaMovies
  def LAAdventureMovies(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfALAAdventureMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LAAdventureMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    if (back == 1) {
      ...
      LAAdventureMovies(huh)
    }
    else {
      println("Please enter a valid number (1-3)." + "\n")
      LAAdventureMoviesButNotStuck(huh)
    }
  }
