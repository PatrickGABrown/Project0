...
object LACrimeMovie {
  ...
  def addingLACrime(yourLAMovieSelection: Int, a: Int): Unit = {
  ...
  //LADramaMovies
  def LACrimeMovies(ahhh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
    ...
    moreOfALACrimeMovie(ahhh)
  }

  //LADramaMovies But You Aren't Stuck:
  def LACrimeMoviesButNotStuck(huh: Int): Unit = {
    val url = "jdbc:mysql://localhost:3306/project0"
    val username = "root"
    val password = "KafeAde!f1a"
    val connection = DriverManager.getConnection(url, username, password)
